...
#define CACHE_X (GRAPH_SIZE_X / 2 + 1)
#define CACHE_Y (GRAPH_SIZE_Y / 2 + 1)

expr_graph_params params;

static float corner_cache[CACHE_Y][CACHE_X];

static double get_corner(te_expr *expr, double *x, double *y, int32_t px, int32_t py)
{
    int32_t cx = px >> 1;
    int32_t cy = (py - GRAPH_POS_Y) >> 1;
    if (!isnan(corner_cache[cy][cx]))
        return corner_cache[cy][cx];
    *x = SCREEN_TO_REAL_X(px);
    *y = SCREEN_TO_REAL_Y(py);
    double val = te_eval(expr);
    corner_cache[cy][cx] = (float)val;
    return val;
}
...
static const int8_t ms_points[16] = {
    ...
    0b1010, 0b1001, 0b0000, 0b1100,
    0b1100, 0b0000, 0b1001, 0b1010,
    0b0110, 0b0101, 0b0011, 0b0000,
};
...
static int32_t linear_interpolate(int32_t p1, int32_t p2, double f1, double f2)
{
    ...
    if ((f1 > 0) != (f2 > 0))
    {
        ...
        int32_t c = p1 + (int32_t)round(t * (p2 - p1));
        return (c < p1) ? p1 : ((c > p2) ? p2 : c);
    }
    return (p1 + p2) / 2;
}
                                  ...
                                  double f1, double f2, double f3, double f4)
{
    ...
    else
    {
        ...
        if (segments & 1) { points[i][0] = cx1; points[i][1] = y1; i++; }
        if (segments & 2) { points[i][0] = x1;  points[i][1] = cy1; i++; }
        if (segments & 4) { points[i][0] = x2;  points[i][1] = cy2; i++; }
        if (segments & 8) { points[i][0] = cx2; points[i][1] = y2; i++; }
...
{
    double f1 = get_corner(expr, x, y, x1, y1);
    double f2 = get_corner(expr, x, y, x2, y1);
    double f3 = get_corner(expr, x, y, x1, y2);
    double f4 = get_corner(expr, x, y, x2, y2);
                                 ...
                                 int32_t depth)
{
    ...
    {
        if (cell_may_contain_curve(expr, x, y, x1, y1, x2, y2))
        {
            double f1 = get_corner(expr, x, y, x1, y1);
            double f2 = get_corner(expr, x, y, x2, y1);
            double f3 = get_corner(expr, x, y, x1, y2);
    ...
    int32_t xm = ((x1 + x2) / 2) & ~1;
    int32_t ym = ((y1 + y2) / 2) & ~1;

    draw_implicit_region(expr, x, y, x1, y1, xm, ym, depth + 1);
    draw_implicit_region(expr, x, y, xm, y1, x2, ym, depth + 1);
    draw_implicit_region(expr, x, y, x1, ym, xm, y2, depth + 1);
    draw_implicit_region(expr, x, y, xm, ym, x2, y2, depth + 1);
}
...
void draw_implicit_expr(te_expr *expr, double *x, double *y)
{
    ...
    memset(corner_cache, 0xFF, sizeof(corner_cache));

    draw_implicit_region(expr, x, y, x1, y1, xm, ym, 0);
    draw_implicit_region(expr, x, y, xm, y1, x2, ym, 0);
    draw_implicit_region(expr, x, y, x1, ym, xm, y2, 0);
    draw_implicit_region(expr, x, y, xm, ym, x2, y2, 0);
}
